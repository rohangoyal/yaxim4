...
import com.namonamo.androidclient.data.RosterProvider.RosterConstants;
import com.namonamo.androidclient.data.YaximConfiguration;
import com.namonamo.androidclient.exceptions.YaximXMPPException;
import com.namonamo.androidclient.util.ConnectionState;
import com.namonamo.androidclient.util.LogConstants;
...
public class SmackableImp implements Smackable {
	...
	final static private String SEND_OFFLINE_SELECTION = ChatConstants.DIRECTION
			...
			+ " OR "
			+ ChatConstants.DELIVERY_STATUS + " = " + ChatConstants.DS_RETRY+")";

	static final DiscoverInfo.Identity YAXIM_IDENTITY = new DiscoverInfo.Identity(
			"client", UIApplication.XMPP_IDENTITY_NAME,
			UIApplication.XMPP_IDENTITY_TYPE);
	...
	private static final String PING_ALARM = "com.namonamo.androidclient.PING_ALARM";
	private static final String PONG_TIMEOUT_ALARM = "com.namonamo.androidclient.PONG_TIMEOUT_ALARM";
	...
	private synchronized void initXMPPConnection() {
		...
		try {
			...
			MemorizingTrustManager mtm = UIApplication.getApp(mService).mMTM;
			sc.init(null, new X509TrustManager[] { mtm },
					new java.security.SecureRandom());
			this.mXMPPConfig.setCustomSSLContext(sc);
			// this.mXMPPConfig
			// .setHostnameVerifier(mtm
			// .wrapHostnameVerifier(new
	...
	private void initServiceDiscovery() {
		...
		dm.addReceiptReceivedListener(new ReceiptReceivedListener() { // DOES
			...
			public void onReceiptReceived(String fromJid, String toJid,
				...
				changeMessageDeliveryStatus(receiptId, ChatConstants.DS_DELIVER);
			}
		});
	}
	...
	public String changePassword(String newPassword) {
		try {
			...
			return "OK"; // HACK: hard coded string to differentiate from
			// failure modes
	...
	public static void sendPendingMessage(ContentResolver cr, String toJID,
			String message) {
		ContentValues values = new ContentValues();
		values.put(ChatConstants.DIRECTION, ChatConstants.OUTGOING);
		values.put(ChatConstants.JID, toJID);
		values.put(ChatConstants.MESSAGE, message);
		values.put(ChatConstants.DELIVERY_STATUS, ChatConstants.DS_PENDING);
		values.put(ChatConstants.DATE, System.currentTimeMillis());
	...
	public void sendMessage(String toJID, String message) {
		...
		if (isAuthenticated()) {
			addChatMessageToDB(ChatConstants.OUTGOING, toJID, message,
					ChatConstants.DS_SENT, System.currentTimeMillis(),
	...
	public void sendPingMessage(String toJID) {
		...
		final Message newMessage = new Message(toJID, Message.Type.chat);
		newMessage.setBody(json.toString());
		// newMessage.addExtension(new DeliveryReceiptRequest());
		if (isAuthenticated()) {
			mXMPPConnection.sendPacket(newMessage);
		}
	...
	protected void sendUserWatching() {
		IQ toggle_google_queue = new IQ() {
			public String getChildElementXML() {
				// enable g:q = start queueing packets = do it when the user is
				// gone
				return "<query xmlns='google:queue'><"
						+ (is_user_watching ? "disable" : "enable")
						+ "/></query>";
			}
		};
			...
			public void processPacket(Packet packet) {
				try {
					if (packet instanceof Message) {
						...
						if (BlockUserDBService.getBlockUser(
								mService.getApplicationContext(), fromJID) != null) {
							return;
						}
						...
						if (checkForPingMessage(chatMessage, fromJID)
						...
						if (direction == ChatConstants.INCOMING) {
							mServiceCallBack.newMessage(fromJID, chatMessage,
									(cc != null));
							if (fromJID.equalsIgnoreCase(_currentJID)) {
								sendPingMessage(fromJID);
							}
						}
					}
				} catch (Exception e) {
	...
	protected boolean checkForPingMessage(String chatMessage, String fromJID) {
		if (chatMessage == null)
			return false;
		try {
			JSONObject json = new JSONObject(chatMessage);
			if (json.getString("type").equalsIgnoreCase("PING")) {
				ContentValues values = new ContentValues();
				values.put(ChatConstants.DELIVERY_STATUS, ChatConstants.DS_SEEN);
				mContentResolver.update(ChatProvider.CONTENT_URI, values,
						ChatConstants.DIRECTION + " = "
								+ ChatConstants.OUTGOING + " AND "
								+ ChatConstants.JID + " = '" + fromJID
								+ "' AND (" + ChatConstants.DELIVERY_STATUS
								+ "=" + ChatConstants.DS_DELIVER + " OR "
								+ ChatConstants.DELIVERY_STATUS + "="
								+ ChatConstants.DS_SENT + ")", null);
			} else {
				return false;
			}
		} catch (Exception x) {
	...
	private ContentValues getContentValuesForRosterEntry(final RosterEntry entry) {
		...
		if (presence.getType() == Presence.Type.error) {
			values.put(RosterConstants.STATUS_MESSAGE, presence.getError()
					.toString());
		} else
	...
	@Override
	public boolean hasRosterItem(String jid) {
		try {
