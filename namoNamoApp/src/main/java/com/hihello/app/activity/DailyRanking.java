...
import com.hihello.app.adapter.AllRankingAdapter;
import com.hihello.app.apicall.GetTopTenDailyRewardApiCall;
...
public class DailyRanking extends Fragment {
	...
	private ServiceListener listener = new ServiceListener() {
		...
		public void onComplete(Servicable<?> service) {
			...
			ArrayList<RewardItem> rewardItems = ((GetTopTenDailyRewardApiCall) service)
					.getAllRewardItems();
			if (rewardItems != null) {
	...
	private void startFetchingAllData() {
		GetTopTenDailyRewardApiCall apiCall = new GetTopTenDailyRewardApiCall(getActivity());
